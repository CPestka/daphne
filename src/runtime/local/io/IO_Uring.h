...
struct URing {
  ...
  URing(uint32_t ring_size, bool use_io_dev_polling, bool use_sq_polling,
      ...
      : use_io_dev_polling(use_io_dev_polling), use_sq_polling(use_sq_polling),
        uuid_counter(0), remaining_slots(ring_size), in_flight_SQEs(ring_size) {
  ...
  uint64_t GetNewUUID(IO_OP_CODE io_op_type) {
    uint64_t uuid = ++uuid_counter;
    return ((uuid << 8) + static_cast<uint8_t>(io_op_type));
  }

  IO_OP_CODE GetIOOPCodeFromUUID(uint64_t uuid) {
    return static_cast<IO_OP_CODE>(static_cast<uint8_t>(uuid & 0xff));
  }

  void Enqueue(const std::vector<URingRead> &reads) {
    read_submission_q.Push(reads);
  }

  void Enqueue(const std::vector<URingWrite> &writes) {
  ...
  void SubmitRead() {
    ...
    std::vector<URingReadInternal> reads =
        read_submission_q.TryPop(read_batch_size);
    uint64_t amount_of_requests_to_submit = reads.size();

    int32_t slots_before_sub =
        remaining_slots.fetch_sub(amount_of_requests_to_submit);
    if (slots_before_sub < amount_of_requests_to_submit) {
      if (slots_before_sub < 0) {
        remaining_slots += amount_of_requests_to_submit;
      } else {
        int32_t amount_to_put_back =
            amount_of_requests_to_submit - slots_before_sub;
        remaining_slots += amount_to_put_back;
        amount_of_requests_to_submit -= amount_to_put_back;
      }
    }

    std::vector<uint64_t> sqe_UUIDs;
    sqe_UUIDs.reserve(amount_of_requests_to_submit);
    ...
    for (size_t i = 0; i < amount_of_requests_to_submit; i++) {
      ...
      io_uring_prep_read(sqe, reads[i].fd, reads[i].current_dest,
                         reads[i].remaining_size, reads[i].offset);

      io_uring_sqe_set_data64(sqe, static_cast<__u64>(sqe_UUIDs[i]));
      ...
      // Set UUID of request, which can be used to reidentify the request once
      // it has been completed
      sqe_UUIDs.push_back(GetNewUUID(IO_OP_CODE::READ));
    }

    uint64_t requests_submitted = use_sq_polling ? sqe_UUIDs.size() : 0;

    if (!use_sq_polling) {
      // Now for all requests for which we got a sqe tell io_uring about them
      uint64_t sqes_to_submit = sqe_UUIDs.size();
      ...
      while (requests_submitted != sqes_to_submit) {
        ...
        if (amount_sqe_submitted > 0) {
          if ((static_cast<uint32_t>(amount_sqe_submitted) +
               requests_submitted) > sqes_to_submit) {
            std::abort();
          }
    ...
    sqe_UUIDs.resize(requests_submitted);

    for (size_t i = 0; i < sqe_UUIDs.size(); i++) {
      in_flight_SQEs.Insert({sqe_UUIDs[i], reads[i].initial_dest,
                             reads[i].current_dest, reads[i].remaining_size,
                             reads[i].result, reads[i].offset, reads[i].fd})
    }
  }
  ...
  void SubmitWrite() {
    ...
    std::vector<URingWriteInternal> writes =
        write_submission_q.TryPop(write_batch_size);
    ...
    for (size_t i = 0; i < amount_of_requests_to_submit; i++) {
      ...
      io_uring_prep_write(sqe, writes[i].fd, writes[i].initial_dest,
                          writes[i].remaining_size, writes[i].offset);
      ...
      // Set UUID of request, which can be used to reidentify the request once
      // it has been completed
      sqe_UUIDs.push_back(GetNewUUID(IO_OP_CODE::WRITE));
    }
    ...
    for (size_t i = 0; i < sqe_UUIDs.size(); i++) {
      in_flight_SQEs.Insert({sqe_UUIDs[i], writes[i].initial_dest,
                             writes[i].current_dest, writes[i].remaining_size,
                             writes[i].write_status, writes[i].offset,
                             writes[i].fd})
    }
  }
  ...
  void HandleRead(InFilghtSQE in_flight_request, int32_t cqe_res) {
    URingReadResult *result =
        static_cast<URingReadResult *>(in_flight_request.result);
  ...
  void HandleWrite(InFilghtSQE in_flight_request, int32_t cqe_res) {
    std::atomic<IO_STATUS> *result =
        static_cast<std::atomic<IO_STATUS> *>(in_flight_request.result);
    ...
    if (cqe_res > 0) {
      ...
      if (cqe_res < in_flight_request.remaining_size) {
        write_submission_q.Push(
            {in_flight_request.initial,
             (static_cast<uint8_t *>(in_flight_request.current) + cqe_res),
             in_flight_request.remaining_size - cqe_res,
             in_flight_request.offset, in_flight_request.fd,
             reinterpret_cast<std::atomic<IO_STATUS> *>(
                 in_flight_request.result)});
        return;
      }
  ...
  void PeekCQAndHandleCQEs() {
    ...
    uint32_t amount_of_fullfilled_requests =
        io_uring_peek_batch_cqe(&ring, cqes, peek_batch_size);

    for (int32_t i = 0; i < static_cast<int32_t>(amount_of_fullfilled_requests);
         i++) {
      IO_OP_CODE current_io_op = GetIOOPCodeFromUUID(cqes[i]->user_data);
      std::optional<InFilghtSQE> in_flight_request =
          in_flight_SQEs.FindAndExtractUUID(cqes[i]->user_data);
