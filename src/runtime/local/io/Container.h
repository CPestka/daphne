...
template <typename T> struct ThreadSafeStack {
  ...
  std::vector<T> TryPop(uint64_t max_elements_poped) {
    ...
    uint64_t elements_to_pop =
        size > max_elements_poped ? max_elements_poped : size;
  ...
  template <bool allready_holding_lck> void Erase(uint64_t id) {
    ...
    std::memcpy(new_buffer + id, data + id + 1,
                sizeof(T) * (size_before_decrement - id - 1));
    free(data);
    data = new_buffer;
    if constexpr (!allready_holding_lck) {
      lck.unlock();
  ...
  ThreadSafeStack() { data = static_cast<T *>(std::malloc(sizeof(T) * 16)); }
  ~ThreadSafeStack() { free(data); }
};
