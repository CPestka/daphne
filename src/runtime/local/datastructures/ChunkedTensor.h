...
public:
    ...
    ChunkedTensor<ValueType>(const std::vector<size_t> &tensor_shape, const std::vector<size_t> &chunk_shape, InitCode init_code)
    ...
    ChunkedTensor<ValueType>(DenseMatrix<ValueType>* matrix, size_t chunk_size_x, size_t chunk_size_y)
        : Tensor<ValueType>(matrix->numRows, matrix->numCols)
    {
        ...
        chunks_per_dim = {
        this->tensor_shape[0] % chunk_size_x == 0 ? this->tensor_shape[0] / chunk_size_x: 1 + (this->tensor_shape[0] / chunk_size_x),
    ...
    explicit ChunkedTensor<ValueType>(ContiguousTensor<ValueType>* other) : Tensor<ValueType>(other->tensor_shape), chunk_shape(other->tensor_shape) {
        chunk_strides.resize(this->rank);
        intra_chunk_strides.resize(this->rank);
    ...
    bool tryRechunk(const std::vector<size_t> &new_chunk_shape) {
        ...
        for (size_t i = 0; i < this->rank; i++) {
            ...
            new_chunk_strides[i] = i == 0 ? new_chunk_element_count : new_chunk_strides[i - 1] * new_chunks_per_dim[i - 1];
        }

        size_t new_total_size_in_elements = new_total_chunk_count * new_chunk_element_count;

        std::shared_ptr<ValueType[]> new_data(new ValueType[new_total_size_in_elements], std::default_delete<ValueType[]>());
        ...
        for (size_t i = 0; i < total_chunk_count; i++) {
            ...
            for (size_t j = 0; j < chunk_element_count; j++) {
                ...
                for (int64_t k = this->rank-1; k >= 0; k--) {
                    current_old_element_ids[static_cast<size_t>(k)] = tmp / intra_chunk_strides[static_cast<size_t>(k)] + current_old_chunk_ids[k] * chunk_shape[k];
                ...
                for (size_t k = 0; k < this->rank; k++) {
                    ...
                    new_linear_id += current_new_chunk_id * new_chunk_strides[k] + current_new_intra_chunk_id * new_intra_chunk_strides[k];
                }
    ...
    ChunkedTensor<ValueType>* tryDiceAtChunkLvl(const std::vector<std::pair<size_t, size_t>> &chunk_ranges) const {
        ...
        if (this->rank == 0) {
            ChunkedTensor<ValueType> *tmp = DataObjectFactory::create<ChunkedTensor<ValueType>>(this->tensor_shape, this->chunk_shape, NONE);
        ...
        ChunkedTensor<ValueType>* new_tensor = DataObjectFactory::create<ChunkedTensor<ValueType>>(new_tensor_shape, chunk_shape, NONE);
        ...
        for (size_t i = 0; i < new_tensor->total_chunk_count; i++) {
            ...
            for (int64_t j = this->rank-1; j >= 0; j--) {
                ...
                old_current_chunk_ids[static_cast<size_t>(j)] = new_current_chunk_ids[static_cast<size_t>(j)] + std::get<0>(chunk_ranges[static_cast<size_t>(j)]);
    ...
    //Ranges are inclusive on both boundaries
    ChunkedTensor<ValueType>* tryDice(const std::vector<std::pair<size_t, size_t>> &index_ranges, const std::vector<size_t> &new_chunk_shape) const {
        if (index_ranges.size() != this->rank || new_chunk_shape.size() != this->rank) {
            return nullptr;
        }

        if (this->rank == 0) {
            ChunkedTensor<ValueType>* tmp = DataObjectFactory::create<ChunkedTensor<ValueType>>(this->tensor_shape, chunk_shape, NONE);
            tmp->data.get()[0] = data.get()[0];
            return tmp;
        }
        ...
        ChunkedTensor<ValueType>* new_tensor = DataObjectFactory::create<ChunkedTensor<ValueType>>(new_tensor_shape, new_chunk_shape, NONE);
    ...
    ContiguousTensor<ValueType>* tryDiceToContiguousTensor(const std::vector<std::pair<size_t, size_t>> &index_ranges) const {
        ...
        if (this->rank == 0) {
            ContiguousTensor<ValueType> *tmp = DataObjectFactory::create<ContiguousTensor<ValueType>>(this->tensor_shape, NONE);
            tmp->data.get()[0] = data.get()[0];
            return tmp;
        }
...
template<typename ValueType>
bool areLogicalElementsEqual(const ContiguousTensor<ValueType> &contiguous_tensor, const ChunkedTensor<ValueType> &chunked_tensor) {
