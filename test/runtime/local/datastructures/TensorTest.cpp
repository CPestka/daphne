...
TEMPLATE_TEST_CASE("Tensor Creation", TAG_DATASTRUCTURES, ALL_VALUE_TYPES) {
  ...
  DataObjectFactory::destroy(ct0,ct1,ct2,ct3,cht0,cht1,cht2,cht3);
}
...
TEMPLATE_TEST_CASE("Tensor layout and accessors", TAG_DATASTRUCTURES, (double,float,uint32_t,uint64_t,int32_t,int64_t)) {
  ...
  std::vector<size_t> chunk_shape_0 = {2, 2, 2};
  std::vector<size_t> chunk_shape_1 = {2, 4, 2, 2};
    ...
    for (size_t i = 0; i < 4; i++) {
      for (size_t j = 0; j < 4; j++) {
        for (size_t k = 0; k < 4; k++) {
          ...
          REQUIRE(ct0->get(ids) == getIOATValue<TestType>(tensor_shape0, ids));
          REQUIRE(cht0->get(ids) == getIOATValue<TestType>(tensor_shape0, ids));
        }
      }
    }
      ...
      for (size_t j = 0; j < 2; j++) {
        for (size_t k = 0; k < 7; k++) {
          for (size_t n = 0; n < 5; n++) {
            ...
            REQUIRE(ct1->get(ids) == getIOATValue<TestType>(tensor_shape1, ids));
            REQUIRE(cht1->get(ids) == getIOATValue<TestType>(tensor_shape1, ids));
          }
        }
      }
    }
    ...
    for (size_t i = 0; i < 4; i++) {
      for (size_t j = 0; j < 4; j++) {
        for (size_t k = 0; k < 4; k++) {
          ...
          REQUIRE(ct0->get(ids) == test_value);
          cht0->set(ids, test_value);
          REQUIRE(cht0->get(ids) == test_value);
      ...
      for (size_t j = 0; j < 2; j++) {
        for (size_t k = 0; k < 7; k++) {
          for (size_t n = 0; n < 5; n++) {
            ...
            REQUIRE(ct1->get(ids) == test_value);
            cht1->set(ids, test_value);
            REQUIRE(cht1->get(ids) == test_value);
